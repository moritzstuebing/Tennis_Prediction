...
The main focus of our prediction modelling was to compare model performance when trained on odds-only data, no-odds data, and a full dataset with odds and all of our chosen features. The models we trained all performed very similarly when trained on all datasets, with often performing slightly better when trained only on odds.
...
In summary, odds are the best predictors for win/loss classification in our dataset, and the addition of features does not significantly improve the performance of classic models.